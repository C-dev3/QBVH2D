...
/// </summary>
public ref struct QBVH2DTraverseIterator
{
    ...
    public bool MoveNext()
    {
        while (!IsStackEmpty())
        {
            ...
            int containsMask = AABB.Contains4(in _point, minX, minY, maxX, maxY);

            // Fold existence check into the containment mask with a single AND (Flags bits 0-3 mark which children exist)
            int mask = containsMask & (node.Flags & 0xF);
    ...
    /// <summary>
    /// Releases the pooled traversal stack back to <see cref="ArrayPool{T}.Shared"/>.
    /// </summary>
    public void Dispose()
    {
        if (!_disposed)
    ...
    /// <summary>
    /// Returns this iterator itself, enabling <c>foreach</c> support without allocating.
    /// </summary>
    /// <returns>This iterator instance.</returns>
    public readonly QBVH2DTraverseIterator GetEnumerator() => this;
}