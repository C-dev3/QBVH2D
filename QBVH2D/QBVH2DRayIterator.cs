...
/// </summary>
public sealed class QBVH2DRayIterator : IEnumerator<int>, IEnumerable<int>, IDisposable
{
    ...
    bool IEnumerator.MoveNext()
    {
        while (!IsStackEmpty())
        {
            ...
            int hitMask = AABB.IntersectsRay4(in _origin, in _invDir, _maxT, in aabb0, in aabb1, in aabb2, in aabb3);

            // Internal node - check all 4 children
            // Process in reverse order so that child 0 is processed first (LIFO stack)
            if ((hitMask & 8) != 0 && node.HasChild(3))
                StackPush(node.GetChildIndex(3));
            if ((hitMask & 4) != 0 && node.HasChild(2))
                StackPush(node.GetChildIndex(2));
            if ((hitMask & 2) != 0 && node.HasChild(1))
                StackPush(node.GetChildIndex(1));
            if ((hitMask & 1) != 0 && node.HasChild(0))
